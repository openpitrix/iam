...
func OpenDatabase(cfg *config.Config) (*Database, error) {
	...
	func() {
		if strings.EqualFold(cfg.DB.Type, "mysql") {
			if !isValidDatabaseName(cfg.DB.Database_AM) {
				logger.Warnf(nil, "invalid db name %s", cfg.DB.Database_AM)
			}

			db, err := sql.Open("mysql", cfg.DB.GetHostUrl_AM())
			if err != nil {
	...
	logger.Infof(nil, "\tDatabase: %s", cfg.DB.Database_AM)
	...
	// open grom
	p.ormDB, err = gorm.Open(cfg.DB.Type, cfg.DB.GetGormUrl_AM())
	if err != nil {
		return nil, err
	}
